...
console.log('\n4b. Customer approves the quotation from the link');
const msg = await ev(B, id => fillTemplate(S.settings.templates.quote, docMessageCtx('quote', get('quotes', id))), ids.quote);
const link = (msg.match(/https?:\/\/\S+approve\.html#[A-Za-z0-9_-]{30,}/) || [])[0];
ok(!!link && /Tap here to approve or decline/.test(msg), 'quotation message carries a private approve link', msg.slice(0, 400));
await syncNow(B);
const cust = await (await b.newContext({ viewport: { width: 390, height: 844 } })).newPage();
cust.on('pageerror', e => errs.push(`customer: ${e.message}`));
await cust.goto(link); await cust.waitForTimeout(1500);
const ct = await cust.textContent('body');
ok(/Quotation QT-/.test(ct) && /Toyota Camry/.test(ct) && /Total/.test(ct), 'customer sees the quotation on their phone', ct.slice(0, 200));
ok(!/050-1234567|demo@example/.test(ct), 'no phone number or e-mail on the page');
await cust.screenshot({ path: `${SHOTS}/l2-11-customer-quote.png`, fullPage: true });
await cust.click('#apv'); await cust.waitForTimeout(500);
ok(/Please type your name/.test(await cust.textContent('#er')), 'name is required');
await cust.fill('#nm', 'Ahmed Al Farsi'); await cust.fill('#nt', 'Please call before starting'); await cust.click('#apv'); await cust.waitForTimeout(1500);
ok(/Approved by Ahmed Al Farsi/.test(await cust.textContent('#act')), 'customer sees "Approved" confirmation');
await cust.screenshot({ path: `${SHOTS}/l2-12-customer-approved.png`, fullPage: true });
await cust.goto(link.replace(/#.*/, '#' + 'x'.repeat(32))); await cust.reload(); await cust.waitForTimeout(1200);
ok(/not valid/.test(await cust.textContent('body')), 'a guessed link shows nothing');
await syncNow(B); await B.waitForTimeout(500);
ok(await ev(B, id => get('quotes', id).status === 'Approved' && get('quotes', id).approval.name === 'Ahmed Al Farsi', ids.quote), 'front desk sees the quote approved online');
ok(/APPROVED by Ahmed Al Farsi/.test(await ev(B, () => document.getElementById('toast').innerText)), '… with a pop-up notice');
await ev(B, id => go('#/quote/' + id), ids.quote); await B.waitForTimeout(700);
ok(/Approved online by Ahmed Al Farsi/.test(await B.textContent('#view')) && /call before starting/.test(await B.textContent('#view')), 'quotation page shows who approved, when, and the note');
await shot(B, 'l2-13-quote-approved');
